...
# Load helper functions (plots, tables, utilities)
source("library_functions.r")
# Load SignalingProfiler wrappers used in the "Extract Protein Activity" tab
source("SignalingProfiler.r")
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      fluidRow(
        column(
          ...
          p(),
          h3(
            "In the ", span("OncoXplorer-CPTAC Data Analysis", style = "font-weight: bold;"), 
            " section, you can compare tumor data from the ", 
            a("CPTAC portal", href = "https://proteomics.cancer.gov/programs/cptac", target = "_blank",
              style = "font-weight: bold; color: #5a9bd5;text-decoration: underline;"), 
            " across different stages and tumor types.", 
            style = "font-size: 1.5em; color: #1A2448;"
          ),
          p(),
          h3(
            "The ", span("Extract Protein Activity from Your Data", style = "font-weight: bold;"), 
            " section allows you to use SignalingProfiler to extract protein activity uploading your data.", 
            style = "font-size: 1.5em; color: #1A2448;"
          ),
          h3(
            "Click here for the ", 
            a("tutorial", href = "tutorial.html", target = "_blank",
              style = "font-weight: bold; color: #5a9bd5;text-decoration: underline;"), 
        ...
        column(
          width = 6,
          img(src = "MultiOmicsXplorer_home.png",
    ...
    tabPanel(
      ...
      tabsetPanel(
          ...
          div(
            ...
            fluidRow(
              column(
                ...
                tags$a(
                  href = "https://pmc.ncbi.nlm.nih.gov/articles/PMC8022323/", 
                  "Simplified and Unified Access to Cancer Proteogenomic Data", 
                  target = "_blank"
                ),
                p("We collected clinical data - used to obtain information about the tumors stages, and raw transcriptomics proteomics and phosphoproteomics data."),
                h3("Pre-processing Steps"),
                p("The raw data underwent several filtering and transformation steps to ensure consistency and usability:"),
                tags$ul(
                  tags$li(strong("Transcriptomics: "), "Filtered out rows with more than 80% zero values and removed non-coding elements."),
                  tags$li(strong("Proteomics: "), "Updated protein identifiers to match Uniprot ID."),
                  tags$li(strong("Phosphoproteomics: "), "Updated Uniprot ID and sequence window based on the phosphorylated residue.")
                ),
              ...
              column(
                ...
                p(
                  ...
                  " package ", 
                  a("(Lombardi et al., 2025)", href = "https://link.springer.com/article/10.1038/s44320-025-00160-y", target = "_blank"),
        ...
        #### Explore Analyte Abundance ####
        tabPanel(
          "Explore Analyte Abundance",
          fluidRow(
            column(
              width = 4,
              radioButtons("data_type","Select one 'omic' layer:",
              ...
              radioButtons(
                "analyte_filter",
                label = tagList("Select analyte filter:", icon("info-circle", id = "info_analyte_filter")),
                choices = c("All Analytes"="all","Significant Analytes"="significant"),
                selected = "all"
              ),
              bsPopover(
                id="info_analyte_filter",
              ...
              selectInput("comparison_type","Select comparison type:",
                          choices = c("Tumor vs Tumor","Tumor vs Non-tumor"),
                          selected = "Tumor vs Tumor"),
              conditionalPanel(
                condition = "input.comparison_type == 'Tumor vs Tumor'",
                selectizeInput("tumors","Select one or more tumors:",
                               choices = c("Brca","Ccrcc","Coad","Gbm","Hnscc","Lscc","Luad","Ov","Pdac","Ucec","all"),
                               selected = "Brca", multiple = TRUE),
                selectizeInput("stages","Select one, more or all stages:",
                               choices = c(1:4,"all"),
                               selected = "all", multiple = TRUE,
                               options = list(id="stage_selector"))
              ),
              conditionalPanel(
                condition = "input.comparison_type == 'Tumor vs Non-tumor'",
                selectizeInput("tumors_norm","Select one tumor:",
                               choices = c("Ccrcc","Coad","Gbm","Hnscc","Lscc","Luad","Ov","Pdac","Ucec","all"),
                               selected = "Ccrcc", multiple = FALSE),
                selectizeInput("stages","Select one, more or all stages:",
                               choices = c(1:4,"all"),
                               selected = "all", multiple = TRUE,
                               options = list(id="stage_selector"))
              ...
              tags$hr(),
              checkboxInput("custom_plot_size","Customize plot size", value = FALSE),
              conditionalPanel(
                condition = "input.custom_plot_size == true",
                numericInput("plot_width_px","Plot width (px)", value = 900, min=400, max=2000, step=50),
                numericInput("plot_height_px","Plot height (px)", value = 500, min=300, max=1400, step=50)
              ),
              div(
            ...
            column(
              ...
              box(
                ...
                withSpinner(uiOutput("combined_plot_ui"), type=4, color="#3399FF", size=0.5),
            ...
            column(
              ...
              selectInput("file_type","Select file type for download:", choices=c("xlsx","csv","tsv")),
              downloadButton("download_data","Download data")
            )
          )
        ),
        
        #### Explore Protein Activity ####
        tabPanel(
          "Explore Protein Activity",
          fluidRow(
            column(
              ...
              conditionalPanel(
                ...
                selectizeInput("tumors_prot","Select one or more tumors:",
                               choices=c("Brca","Ccrcc","Coad","Gbm","Hnscc","Lscc","Luad","Ov","Pdac","Ucec","all"),
                               selected="Brca", multiple=TRUE),
                selectizeInput("stages_prot","Select one, more or all stages:",
                               choices=c(1:4,"all"), selected="all", multiple=TRUE,
                               options=list(id="stage_selector_prot"))
              ),
              conditionalPanel(
                condition = "input.comparison_type_protein == 'Tumor vs Non-tumor'",
                selectizeInput("tumors_prot","Select one tumor:",
                               choices=c("Ccrcc","Coad","Gbm","Hnscc","Lscc","Luad","Pdac","Ov","Ucec","all"),
                               selected="Ccrcc", multiple=FALSE),
                selectizeInput("stages_prot","Select one, more or all stages:",
                               choices=c(1:4,"all"), selected="all", multiple=TRUE,
                               options=list(id="stage_selector_prot"))
              ...
              tags$hr(),
              checkboxInput("custom_plot_size_prot","Customize plot size", value=FALSE),
              conditionalPanel(
                condition = "input.custom_plot_size_prot == true",
                numericInput("plot_width_px_prot","Plot width (px)", value=900, min=400, max=2000, step=50),
                numericInput("plot_height_px_prot","Plot height (px)", value=500, min=300, max=1400, step=50)
              ),
              div(
            ...
            column(
              ...
              box(
                width=12,
                style="min-height: 520px;",
                withSpinner(uiOutput("combined_plot_prot_ui"), type=4, color="#3399FF", size=0.5),
                class="plot-container",
                downloadButton("download_pdf_prot","Download plot (PDF)",
                               class="btn btn-default btn-sm", style="margin-top:10px;")
            ...
            column(
              ...
              selectInput("file_type_prot","Select file type for download:", choices=c("xlsx","csv","tsv")),
    ...
    tabPanel(
        ...
        tabPanel(
          ...
          div(
            ...
            div(
              ...
              br(),
              p("If you are unsure how to generate these properly formatted omics tables, you can use the following preparation script."),
              
              tags$a(
                href = "script_omics_preparation.R",
                "Click here to download the omics table preparation script",
                target = "_blank",
                download = NA
              ),
        ...
        tabPanel(
          ...
          fluidRow(
            column(
              ...
              tags$h5("Example input files"),
              
              tags$p(
                "You can download three example datasets (sample ID: CPT000814) ",
                "to understand the required input structure:"
              ),
              
              tags$ul(
                tags$li(
                  tags$a(href = "P_CPT000814.tsv",
                         "P_CPT000814.tsv",
                         target = "_blank")
                ),
                tags$li(
                  tags$a(href = "Ph_CPT000814.tsv",
                         "Ph_CPT000814.tsv",
                         target = "_blank")
                ),
                tags$li(
                  tags$a(href = "T_CPT000814.tsv",
                         "T_CPT000814.tsv",
                         target = "_blank")
                )
              ),
     ...
     <br><br>
     In addition, it is important that your data includes correct UniProt IDs and valid amino acids (S, T, or Y). 
     Otherwise, the dataset will undergo a <strong>partial cleanup</strong> and some entries may be removed.
     <br><br>
     An example preprocessing script is available here: 
     <a href='script_omics_update.R' target='_blank' style='color: #d35400; font-weight: 600;'>
     Download preprocessing script</a>.
     <br><br>
     For more advanced preprocessing options, refer to the 
     <a href='https://github.com/SaccoPerfettoLab/PatientProfiler' target='_blank' style='color: #f39c12;'>
     PatientProfiler</a> package."
                )
              ),
              ...
              div(
                ...
                tags$div(
                  ...
                  icon("info-circle", id = "info_organism"),
            ...
            column(
              ...
              box(
                ...
                withSpinner(DTOutput("results_table"), type = 4, color = "#3399FF", size = 0.5),
                div(style = "text-align: center; margin-top: 15px;",
                    downloadButton("download_results", "Download Results", class = "btn-primary")),
                p(style = "font-weight: bold; color: #3ac554; margin-top: 10px; text-align: center;",
                ...
                withSpinner(DTOutput("summary_table"), type = 4, color = "#3399FF", size = 0.5),
                div(style = "text-align: center; margin-top: 15px;",
                    downloadButton("download_summary", "Download Summary", class = "btn-info")),
                br(), br(),
                hr(style = "border-top: 2px solid #D3D3D3;"),

                h4("Regulons Evidence Table", style = "text-align: center; margin-bottom: 10px;"),
                withSpinner(DTOutput("regulons_table"), type = 4, color = "#3399FF", size = 0.5),
                div(style = "text-align: center; margin-top: 15px;",
                    downloadButton("download_regulons", "Download Regulons", class = "btn-warning")),
                br(), br(),
                hr(style = "border-top: 2px solid #D3D3D3;")
        ...
        tabPanel(
          "Plot Results",
          h3("View the Top Up and Down-Regulated Proteins Based on Their Predicted Activity"),
          p(style = "text-align: justify;", 
          ...
          fluidRow(
            column(4, selectInput("top_n", "Show", choices = c("Top 10" = "10", "Top 15" = "15", "Top 25" = "25", "Top 50" = "50", "All" = "all"), selected = "15")),
            column(4, numericInput("plot_width", "Plot width (inches)", value = 10, min = 4, max = 20)),
            column(4, numericInput("plot_height", "Plot height (inches)", value = 8, min = 4, max = 20))
          ),
          actionButton("generate_plot_sp", "Generate Plot"),   # <<< rinominato
          downloadButton("download_plot", "Download Plot"),
          tags$br(),
          withSpinner(
            uiOutput("protein_plot_ui"),
      ...
      fluidRow(
        column(
          ...
          tags$ul(
            ...
            tags$li("Head of the Lab: Professor Livia Perfetto"),
            tags$li(
              "Primary curator of MultiOmicsXplorer: Dr. Eleonora Meo (",
              a("eleonorameo.hp@gmail.com", href = "mailto:eleonorameo.hp@gmail.com"),
              ")"
            ),
            tags$li(
              "Maintainer & Scientific correspondence: Dr. Veronica Lombardi (",
              a("veronica.lombardi@uniroma1.it", href = "mailto:veronica.lombardi@uniroma1.it"),
              ")"
            )
  ...
  tags$footer(
    div(
      ...
      div(
        ...
        "For support, contact: ",
        a("eleonorameo.hp@gmail.com", href = "mailto:eleonorameo.hp@gmail.com",
          style = "color: #939393; font-weight: bold; text-decoration: none;")
      )
    ),
    div(
...
server <- function(input, output, session) {
  ...
  #### for the code on server ####
  Sys.setenv(PATH = paste("/home/livia/ncbi-blast-2.16.0+/bin", Sys.getenv("PATH"), sep = ":"))
  ...
  data_reactive <- reactive({
    ...
    if (comparison_type == "Tumor vs Tumor") {
      stages <- input$stages
    } else if (comparison_type == "Tumor vs Non-tumor") {
      stages <- NULL  
    } else {
      return(NULL)
    }
  ...
  p <- eventReactive(input$generate_plot, {
    ...
    # add class for clicked button to start animation
    shinyjs::addClass(id = "generate_plot", class = "clicked")
    
    # remove class for button animation after 1 sec
    shinyjs::delay(1000, {
      shinyjs::removeClass(id = "generate_plot", class = "clicked")
    })
    
    # verify selection of analyte (gene)
    if (is.null(input$gene) || input$gene == "" || input$gene == "- select") {
      return(
        ggplot() +
          theme_void() +
          annotate("text", x = 0.5, y = 0.5,
                   label = "Please select an analyte",
                   size = 6, color = "red")
      )
    }
    
    # obtain parameters from UI
    data_type <- tolower(input$data_type)
    gene <- input$gene
    comparison_type <- input$comparison_type
    ...
    # if data are null don't do anything
    req(data)
    ...
    if (comparison_type == "Tumor vs Tumor") {
      ...
      stages <- input$stages
      plot_obj <- generate_plot_abundance(
        data,
        gene,
        tumor_types,
        stages,
    ...
    # handle no data
    if (is.null(plot_obj) || (is.data.frame(plot_obj$data) && nrow(plot_obj$data) == 0)) {
      showNotification("No data available for the selected tumor types and stages.",
  ...
  # reactive width/height (default if customization OFF)
  
  plot_width_px <- reactive({
    if (isTRUE(input$custom_plot_size)) input$plot_width_px else 900
  })
  
  plot_height_px <- reactive({
    if (isTRUE(input$custom_plot_size)) input$plot_height_px else 500
  })
  
  
  # dynamic plotOutput in UI 
...
#### PhosphoSIGNOR link generation ####
  observeEvent(input$generate_plot_sp, {
    plot_obj <- p()  
    req(plot_obj)  
    
    output$signor_info <- renderUI(NULL)  # reset of the Signor link every time generate plot is clicked
...
#### Populate analyte selector based on omic layer ####
...
#### Download: analyte abundance plot ####

  output$download_pdf <- downloadHandler(
    filename = function() {
      paste0("abundance_plot_", Sys.Date(), ".pdf")
    },
    content = function(file) {
      ...
      w_px <- if (isTRUE(input$custom_plot_size) && !is.null(input$plot_width_px))  input$plot_width_px  else 900
      h_px <- if (isTRUE(input$custom_plot_size) && !is.null(input$plot_height_px)) input$plot_height_px else 500
      
      # px -> inch (96 px per inch)
      width_in  <- max(2, w_px / 96)
      height_in <- max(2, h_px / 96)
      
      
      scale_factor <- min(width_in / (900/96), height_in / (500/96))
      scale_factor <- max(0.5, min(scale_factor, 1.2))
      
      plot_pdf <- plot_obj + theme(
        plot.title   = element_text(size = 16 * scale_factor),
        axis.title   = element_text(size = 14 * scale_factor),
        axis.text    = element_text(size = 12 * scale_factor),
        legend.title = element_text(size = 12 * scale_factor),
        legend.text  = element_text(size = 10 * scale_factor)
      )
...
#### Table: analyte abundance ####
  table_data <- eventReactive(input$generate_plot, {
    ...
    data <- load_data(data_type, gene)
    req(data)  # be sure data is not null
    ...
    if (comparison_type == "Tumor vs Tumor") {
      ...
      # if "all" is selected for the stage do not filter for stages
      filtered_data <-
...
#### Download: analyte abundance table ####
...
#### Plot: protein activity ####
  ...
  p_prot <- eventReactive(input$generate_protact_plot, {
    ...
    if (is.null(protein) || is.null(comparison_type_prot)) return(NULL)
    ...
    if (comparison_type_prot == "Tumor vs Tumor") {
      ...
      if ("all" %in% tumors) tumors <- unique(data$Tumor_Type)
      stages <- input$stages_prot
    ...
    } else if (comparison_type_prot == "Tumor vs Non-tumor") {
      req(input$protein, input$tumors_prot, input$stages_prot)
  ...
  # reactive width/height (PROT)

  prot_plot_width_px <- reactive({
    if (isTRUE(input$custom_plot_size_prot)) input$plot_width_px_prot else 900
  })
  
  prot_plot_height_px <- reactive({
    if (isTRUE(input$custom_plot_size_prot)) input$plot_height_px_prot else 500
  })
  

  # dynamic plotOutput (PROT)

  output$combined_plot_prot_ui <- renderUI({
    plotOutput(
      outputId = "combined_plot_prot",
      width  = paste0(prot_plot_width_px(), "px"),
      height = paste0(prot_plot_height_px(), "px")
    )
  })
  
  # Output plot protein activity
...
#### Download: protein activity plot ####
  ...
  output$download_pdf_prot <- downloadHandler(
    ...
    content = function(file) {
      ...
      w_px <- prot_plot_width_px()
      h_px <- prot_plot_height_px()
      ...
      base_w_in <- 900 / 96
      base_h_in <- 500 / 96
      scale_factor <- min(w_in / base_w_in, h_in / base_h_in)
      scale_factor <- max(0.5, min(scale_factor, 1.2)) 
      
      plot_pdf <- p_prot() + ggplot2::theme(
        plot.title   = ggplot2::element_text(size = 16 * scale_factor),
        axis.title   = ggplot2::element_text(size = 14 * scale_factor),
        axis.text    = ggplot2::element_text(size = 12 * scale_factor),
        legend.title = ggplot2::element_text(size = 12 * scale_factor),
        legend.text  = ggplot2::element_text(size = 10 * scale_factor)
      )
...
#### Download: protein activity table ####
...
#### SignalingProfiler: activity inference from uploaded files, example input tables (About tab) 
...
#### SignalingProfiler: upload and validate files ####
  ...
  # NEW: regulons evidence table across samples
  all_regulons <- reactiveVal(
  data.frame(
    sample_id = character(),
...
#### SignalingProfiler: run inference for each sample ####
    ...
    for (sample_id in names(sample_data)) {
      ...
      if (is.null(prot_data)) {
        if (params$correct_proteomics_tf || params$correct_proteomics_kin) {
          ...
          # Disabilita la correzione solo per questo sample
      ...
      if (!is.null(analysis_output$regulons) && nrow(analysis_output$regulons) > 0) {
        all_regulons_data <- bind_rows(all_regulons_data, analysis_output$regulons)
      }
    }
    
    all_results(all_results_data)
    all_summaries(temp_summaries)
    all_regulons(all_regulons_data)
...
#### Output: results and summary tables ####
  ...
  # Regulons evidence table
  output$regulons_table <- renderDataTable({
    all_regulons()
  })

  output$download_regulons <- downloadHandler(
    filename = function() {
      paste0("regulons_evidence_", Sys.Date(), ".csv")
    },
    content = function(file) {
      write.csv(all_regulons(), file, row.names = FALSE)
    },
...
#### SignalingProfiler: clear all loaded data/results ####
  ...
  data.frame(
    ...
    output$regulons_table <- renderDataTable({ all_regulons() })
  })
  
  
  
#### SignalingProfiler: plot results (Top up/down activities) ####
...
#### SignalingProfiler: generate plot ####
  # UI container that follows the selected plot height
  output$protein_plot_ui <- renderUI({
    req(input$plot_height)
    plotOutput("protein_plot",
               width = "100%",
               height = paste0(input$plot_height * 72, "px"))
  })
  ...
  observeEvent(input$generate_plot_sp, {
    ...
    # Build plot with user-selected Top N
    top_n_val <- if (identical(input$top_n, "all")) Inf else as.numeric(input$top_n)
    
    plot <- create_top_proteins_plot(
    ...
    width  = function() input$plot_width * 72,
    height = function() input$plot_height * 72)
    
    # Download as PDF using the same dimensions
    output$download_plot <- downloadHandler(
      filename = function() {
        paste0("protein_activity_plot_", Sys.Date(), ".pdf")
      },
      content = function(file) {
